...
_AI_IMAGE_PROMPTS = {
    "real": [
        "a real photograph taken by a camera with natural lighting and imperfections",
        "a genuine photo of an actual product with realistic shadows depth and texture",
        "an authentic real-world image with natural photographic characteristics",
    ],
    "ai": [
        "an AI-generated image created by artificial intelligence or diffusion model like Midjourney",
        "a synthetic image generated by Stable Diffusion DALL-E or similar AI art generator",
        "an artificially generated image that looks too perfect and was not photographed by a camera",
    ],
}
_ai_image_cache: dict[str, np.ndarray] = {}


async def _load_ai_image_prompts():
    ...
    def _encode():
        result = {}
        for key, prompts in _AI_IMAGE_PROMPTS.items():
            inputs = _clip_processor(text=prompts, return_tensors="pt",
            ...
            with torch.no_grad():
                ...
                result[key] = emb.mean(dim=0).cpu().numpy()
        return result

    try:
        _ai_image_cache.update(await loop.run_in_executor(None, _encode))
        return True
    except Exception as e:
        print(f"[ReCheck] AI 이미지 감지 프롬프트 로드 실패: {e}")
        return False


async def detect_ai_image(image_bytes: bytes) -> dict:
    """사진이 AI 생성 이미지인지 CLIP 임베딩으로 판별"""
    if not _clip_model:
        return {"is_ai_generated": False, "ai_probability": 0.0, "level": "unknown"}
    if not _ai_image_cache:
        await _load_ai_image_prompts()
    if not _ai_image_cache:
        return {"is_ai_generated": False, "ai_probability": 0.0, "level": "unknown"}

    import math
    img = PILImage.open(io.BytesIO(image_bytes)).convert("RGB")
    img_emb = await _get_image_embedding(img)
    if img_emb is None:
        return {"is_ai_generated": False, "ai_probability": 0.0, "level": "unknown"}

    real_score = float(np.dot(img_emb, _ai_image_cache["real"]))
    ai_score   = float(np.dot(img_emb, _ai_image_cache["ai"]))
    exp_r, exp_a = math.exp(real_score * 10), math.exp(ai_score * 10)
    ai_prob = round(exp_a / (exp_r + exp_a) * 100, 1)
    ...
    return {"is_ai_generated": ai_prob >= 70, "ai_probability": ai_prob, "level": level}


# ── 이미지 지문 (중복 탐지) ──────────────────────────────────────────
def _compute_phash(img: PILImage.Image, hash_size: int = 8) -> str:
    """퍼셉추얼 해시 — 유사 이미지는 해밍 거리가 작음"""
    gray = img.convert("L").resize((hash_size, hash_size), PILImage.LANCZOS)
...
async def check_duplicate_image(image_bytes: bytes, db_path: str) -> dict:
    """ReCheck에서 이전에 분석된 동일/유사 이미지가 있는지 확인"""
    import sqlite3
    img = PILImage.open(io.BytesIO(image_bytes)).convert("RGB")
    img_emb = await _get_image_embedding(img)
    new_phash = _compute_phash(img)
    if img_emb is None:
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            rows = conn.execute(
                "SELECT phash, clip_embedding, listing_url, brand, model_name, analyzed_at "
                "FROM image_fingerprints ORDER BY analyzed_at DESC LIMIT 2000"
            ).fetchall()
    except Exception:
        return {"is_duplicate": False, "similarity": 0.0}

    best_sim, best_row = 0.0, None
    for phash, emb_blob, listing_url, brand, model_name, analyzed_at in rows:
        if _phash_distance(new_phash, phash) > 20:
            continue
        stored_emb = np.frombuffer(emb_blob, dtype=np.float32)
        sim = float(np.dot(img_emb, stored_emb))
        if sim > best_sim:
            best_sim = sim
            best_row = (listing_url, brand, model_name, analyzed_at)

    is_dup = best_sim >= 0.92
    result: dict = {"is_duplicate": is_dup, "similarity": round(best_sim * 100, 1)}
    if best_row:
        result["listing_url"]  = best_row[0] or ""
        result["brand"]        = best_row[1] or ""
        result["model_name"]   = best_row[2] or ""
        result["analyzed_at"]  = best_row[3] or ""
    return result


async def store_image_fingerprint(image_bytes: bytes, db_path: str,
                                   listing_url: str = "", brand: str = "",
                                   model_name: str = ""):
    """분석한 이미지의 지문을 DB에 저장 (이후 중복 탐지에 활용)"""
    import sqlite3
    img = PILImage.open(io.BytesIO(image_bytes)).convert("RGB")
    img_emb = await _get_image_embedding(img)
    ...
    if img_emb is None:
        return
    emb_blob = img_emb.astype(np.float32).tobytes()
    try:
        with sqlite3.connect(db_path) as conn:
            conn.execute(
                "INSERT INTO image_fingerprints (phash, clip_embedding, listing_url, brand, model_name) "
                "VALUES (?,?,?,?,?)",
                (phash, emb_blob, listing_url, brand, model_name),
            )
            conn.commit()
    except Exception as e:
        print(f"[ReCheck] 이미지 지문 저장 실패: {e}")
